...
from compydre import comadre_transformer
...
def mock_species_matrix(mock_matrices, mock_metadata):
    return comadre_transformer.SpeciesMatrix("species1", mock_matrices, None, mock_metadata, 0)


@pytest.fixture
def mock_study_info():
    arr = np.array([['Calf: 0-1 years', '240296'],
...
@pytest.fixture
def mock_metadata():
    arr = np.array([["This is a mock", "I am mocking", "Don't mock the boat"] for i in range(0,6)])
...
def mock_lc_graph(mock_study_info, mock_matrices):
    ...
    edge_list = [('Adult: 2+ years', 'Calf: 0-1 years', 1.12),
...
def test_species_matrix(mock_matrices, mock_study_info, mock_metadata):
    ...
    actual = comadre_transformer.SpeciesMatrix(test_species, mock_matrices, mock_study_info, mock_metadata, test_index)
...
def test_project_species_growth(mock_matrices, mock_study_info, mock_metadata):
    ...
    test_class = comadre_transformer.SpeciesMatrix(test_species, mock_matrices, mock_study_info, mock_metadata, test_index)
...
def test_return_lc_graph(mock_matrices, mock_study_info, mock_lc_graph, mock_metadata):
    ...
    test_class = comadre_transformer.SpeciesMatrix(test_species,
                                                   mock_matrices,
                                                   mock_study_info,
                                                   mock_metadata,
                                                   test_index)
...
def test_comadre_transformer():

    test_class = comadre_transformer.ComadreTransformer("path")
...
def test_comadre_read(mocker):
    mock_r = mocker.patch("compydre.comadre_transformer.r")
    mock_comadre = mocker.patch("compydre.comadre_transformer.r.comadre", 2)
    test_class = comadre_transformer.ComadreTransformer("path")
...
def test_create_species_dict(mocker, mock_matrices, mock_species_matrix, mock_species_data, mock_metadata):
    ...
    mock_species_matrix = mocker.patch.object(comadre_transformer, "SpeciesMatrix")
    ...
    mock_rx2_call.rx2.return_value = ["species1", "species1", "species1",
...
def test_return_matrices(mocker):
    test_class = comadre_transformer.ComadreTransformer("path")
    mock_raw_data = mocker.patch.object(test_class, "_raw_data")
    mock_raw_data.rx2.return_value = [[[0, 1, 2], [[0, 0], [0, 0]],
                                       [7, 8, 9]],
...
def test_return_species(monkeypatch, mock_species_data, exists):
    monkeypatch.setattr(comadre_transformer.ComadreTransformer, "species_data", mock_species_data)
