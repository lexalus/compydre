...
class SpeciesMatrix(object):
    ...
    def show_lc_graph(self):
        ...
        # labels
        nx.draw_networkx_labels(self.lc_graph, pos, font_size=10, font_family="sans-serif")
...
class ComadreTransformer(object):
    ...
    def return_species(self, species):
        try:
            ...
            similar = list(map(self.species_data.get, filter(lambda x: species in x, self.species_data)))
    ...
    def return_study_info(self, species_index):
        study_info = pd.DataFrame(np.array(self.raw_data.rx2("matrixClass")[species_index]).T,
                                  columns=["MatrixClassOrganized", "MatrixClassAuthor",
                                           "MatrixClassNumber", "MatrixId"])
        study_info = study_info[["MatrixClassAuthor", "MatrixId"]]
        study_info.columns = ["StudyPopulationGroup", "MatrixId"]
        return study_info

    def return_metadata(self):
        metadata =  pd.DataFrame(np.array(self.raw_data.rx2("metadata")).T,
    ...
    def create_species_dict(self):
        ...
        for i in species_list:
            ...
            matrices = {MATRIX_TYPES[i]: self.return_matrices(curr, i) for i in range(0, 4)}
            index = curr
            study_info = self.return_study_info(curr)
            metadata = self._metadata.iloc[curr]

            species_dict[i].append(SpeciesMatrix(i, matrices, study_info, metadata, index))
            curr += 1
